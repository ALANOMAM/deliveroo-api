...
@section('content')
    ...
    <div class="container col-9 py-5">
        ...
        <a href="{{url('admin/order-stats2')}}" class="nav-link link-no-active " >
          <span style="color:#eb6b3e"><strong>Vai al grafico(mesi-ordini)</strong> </span>           
        </a>
...
/* const data = [
  ...
  //associo al numero del mese estratto  un nome del mese preso dall'array sopra
  let month_name = month_names[orders_in_js_decoded2[i].month_number]
 
  if(orders_in_js_decoded2[i].month_number <= 5){
    data.push({ month:month_name , orders_number:orders_in_js_decoded2[i].orders_per_month });
  }
      }
  console.log(data); 
 ...
 new Chart(ctx2, {
    ...
    data: {
      ...
      datasets: [{
       ...
       //dato nell'asse delle y 
     //data: [65, 59, 80, 81, 56, 55, 40, 100],
     data: data.map(row => row.orders_number),
